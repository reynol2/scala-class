 ...
 */
 @RunWith(classOf[JUnitRunner])
  ...
  test("max of many numbers") {
    assert(max(List(51, 10, 13, 12, 0, 1, 1000, 40, 1)) === 1000)
  }

  test("max of negative numbers") {
    assert(max(List(-12, -1, 0, -1000, -25, -2)) === 0)
  }

  test("sum of negative numbers") {
    assert(sum(List(-2, -3, -100, -20, -75)) == -200)
  }

  test("sum and max used in conjunction") {
    assert(sum(List(max(List(3, 10, 1)), max(List(2, 4, 6, 8)))) === 18)
  }

  test("max throws an exception with empty lists") {
